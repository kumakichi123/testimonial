  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="card w-full space-y-10 p-6 sm:p-10">
        ...
        <form action={`/api/submit/${slug}`} method="post" className="space-y-6">
          ...
          <FormField label={<span>コメント{REQUIRED_MARK}</span>}>
            <textarea name="comment" required className={`${inputClassName} min-h-[7.5rem] resize-y`} />
          </FormField>
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-5 sm:px-6">
            <label className="flex items-start gap-3 text-left text-sm sm:text-base">
              ...
              <span className="text-slate-700">
                <span className="font-semibold">公開される可能性への同意{REQUIRED_MARK}</span>
                <br />
                サービス改善や事例紹介として回答内容が公開される可能性があることに同意します。
              </span>
            </label>
          </div>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button className="btn primary w-full justify-center text-base sm:w-auto" type="submit">
              送信する
            </button>
          </div>
        </form>
